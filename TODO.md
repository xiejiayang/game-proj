...
| 编号 | 任务 | 优先级 | 状态 | 验收标准 | 关联规范 |
|---|---|---|---|---|---|
| M2.1.1 | 底部 Toolbar 实现竹笼、杩槎放置 | P0 | 未开始 | ToolbarItem 60×60px，选中态、禁用态、计数显示符合 DESIGN §5.5 | DESIGN §5.5 |
| M2.1.2 | 实现拖拽放置的完整反馈（ghost、吸附、非法回退） | P0 | 未开始 | 按下即出 ghost；释放合法则 150ms 吸附动画；非法变红 200ms 后回 toolbar；释放到 canvas 外不消耗库存 | DESIGN §6.3 |
| M2.1.3 | 实现撤销（Undo）与重置（Reset） | P1 | 未开始 | Undo 回退一步编辑操作；Reset 恢复关卡初始布局 | PRD §3.2 |
| M2.1.4 | 实现实时资源消耗显示（料/工/时） | P1 | 未开始 | HUD 顶部显示三种资源，ResourceDot 颜色符合 DESIGN §5.6 | PRD §3.2、DESIGN §5.6 |
| M2.1.5 | 空操作提示（未放构件点击放水） | P2 | 未开始 | 顶部 HintPill 显示「先在河里摆上构件」，2.5s 淡出 | DESIGN §7.2 |

### M2.2 模拟与结算

| 编号 | 任务 | 优先级 | 状态 | 验收标准 | 关联规范 |
|---|---|---|---|---|---|
| M2.2.1 | 完善 WaterInteraction 效果 | P0 | 未开始 | 石墙反弹+耐久伤害；竹笼/杩槎按法线分流；构件被冲毁 300ms 淡出 | PRD §6.3、DESIGN §6.2 |
| M2.2.2 | 实现成功/失败/节俭结算逻辑 | P0 | 未开始 | 成功条件：模拟时长 ≥ targetDuration 且村庄未淹没；失败条件：Flood / Destroyed / Timeout；节俭阈值判定正确 | PRD §5.4 |
| M2.2.3 | 实现结算弹窗第一屏（成功/失败） | P0 | 未开始 | 成功：印章「安」绿色，标题「暂时安全」，节俭/非节俭文案，按钮「查看碑廊」；失败：印章「倒」红色，标题「村子仍被淹」，按钮「再试一次」 | DESIGN §7.5、§7.6 |
| M2.2.4 | 实现结算弹窗第二屏（碑廊解锁） | P1 | 未开始 | 标题「堵不如疏」，图文内容，按钮「再试一次」+「下一关（占位）」 | PRD §5.1、DESIGN §7.5 |
| M2.2.5 | 结算页切换动画（屏1 → 屏2） | P2 | 未开始 | 300ms ease-in-out 淡入淡出 | DESIGN §6.2 |

### M2.3 提示系统

| 编号 | 任务 | 优先级 | 状态 | 验收标准 | 关联规范 |
|---|---|---|---|---|---|
| M2.3.1 | 实现老河工提示对话框 | P1 | 未开始 | 底部居中，头像 + 文字，打字机效果 40ms/字，关闭按钮或 5s 自动关闭 | DESIGN §7.3 |
| M2.3.2 | 实现提示触发逻辑 | P1 | 未开始 | 第 3 次失败触发第一层，第 5 次失败触发第二层；不开放第三层 | PRD §5.2、ARCH §F7 |
| M2.3.3 | 提示出现动画 | P2 | 未开始 | 400ms ease-out 从底部滑入 + 淡入 | DESIGN §6.2 |

### M2.4 暂停与设置

| 编号 | 任务 | 优先级 | 状态 | 验收标准 | 关联规范 |
|---|---|---|---|---|---|
| M2.4.1 | 实现暂停菜单 | P1 | 未开始 | 全屏 `--paper` 95% 遮罩；按钮：继续、重试、设置、返回标题 | PRD §4、DESIGN §7.4 |
| M2.4.2 | 实现设置页（音量、画质、语言预留） | P1 | 未开始 | 音乐、音效滑块；画质下拉（低/中/高）；语言预留字段；设置保存到 PlayerProfile | PRD §3.2、DESIGN §7 |
| M2.4.3 | 暂停菜单出现动画 | P2 | 未开始 | 200ms 遮罩淡入 + 按钮依次上移 50ms stagger | DESIGN §6.2 |
...
| 编号 | 任务 | 优先级 | 状态 | 验收标准 | 关联规范 |
|---|---|---|---|---|---|
| M3.1.1 | 替换灰盒为低多边形 3D 场景 | P0 | 未开始 | 地形、山石、村庄、河道使用低多边形模型；单模型面数 ≤ 500 | DESIGN §9.1 |
| M3.1.2 | 场景使用淡墨+青绿色调材质，饱和度 ≤ 40% | P0 | 未开始 | 无写实高反光材质；整体色调符合 `--paper`、`--ink-*`、`--accent` | DESIGN §2、§9.1 |
| M3.1.3 | 水面片与河道视觉效果 | P1 | 未开始 | 水面使用 `--water` 色系，配合简单波动或 Shader | DESIGN §2 |

### M3.2 水墨 Shader 与后处理

| 编号 | 任务 | 优先级 | 状态 | 验收标准 | 关联规范 |
|---|---|---|---|---|---|
| M3.2.1 | 使用 URP Shader Graph 实现水墨材质（宣纸纹理、边缘晕染、墨色渐变） | P0 | 未开始 | 应用到场景基底、构件上；肉眼可识别水墨感 | PRD §6.4、ARCH §1.1 |
| M3.2.2 | 实现全局后处理：水墨勾边、纸质噪点、远景雾化 | P1 | 未开始 | 后处理不造成帧率低于 30fps；避免 WebGL 不支持的效果（复杂屏幕空间反射） | PRD §6.4、ARCH §6.4 |
| M3.2.3 | 实现水粒子 Trail Renderer + 水墨 Shader | P1 | 未开始 | 速度越快墨色越浓、拖尾越长；粒子数 ≤ 200 | PRD §6.3、DESIGN §9.2 |
| M3.2.4 | 性能优化：必要时动态降质（粒子数、后处理强度） | P1 | 未开始 | 中低端手机稳定 30fps 以上 | PRD §7.1、ARCH §6.1 |

### M3.3 2D 叙事资产

| 编号 | 任务 | 优先级 | 状态 | 验收标准 | 关联规范 |
|---|---|---|---|---|---|
| M3.3.1 | 生成/收集 L1 引子叙事图（AI 水墨风格） | P2 | 未开始 | 统一关键词：水墨、淡彩、宣纸、宋代山水、留白；存放于 `Assets/_Project/Art/AI_Generated/Narrative/` | PRD §6.7、ARCH §5、DESIGN §9.3 |
| M3.3.2 | 生成老河工头像/人物剪影（AI 水墨风格） | P2 | 未开始 | 存放于 `Assets/_Project/Art/AI_Generated/Characters/` | ARCH §5.2 |
| M3.3.3 | 为每个 AI 生成文件创建元数据 `.md` | P2 | 未开始 | 记录工具、prompt、修改历史、商用授权说明 | ARCH §5.2 |
| M3.3.4 | UI 装饰元素（印章、水墨纹理、进度条素材） | P2 | 未开始 | 优先程序生成宣纸噪点；贴图尺寸为 2 的幂次方，最大 1024px | DESIGN §9.4 |
...
| 编号 | 任务 | 优先级 | 状态 | 验收标准 | 关联规范 |
|---|---|---|---|---|---|
| M4.1.1 | 启动/标题页 | P0 | 未开始 | Logo、水墨背景、「开始治水」按钮、设置入口；两页合并为一页 | PRD §4、DESIGN §7 |
| M4.1.2 | L1 引子页 | P0 | 未开始 | 官方筑墙失败、村庄危急叙事；可跳过；400–600ms 水墨过渡 | PRD §4、§5.1、DESIGN §6.2 |
| M4.1.3 | 游戏主界面 | P0 | 未开始 | 3D 河道场景 + HUD（关卡名、阶段、资源条、底部 Toolbar） | PRD §4 |
...
| M4.1.8 | 加载页 | P1 | 未开始 | 水墨进度条 + 文案「研墨中…」；不显示百分比但进度真实 | DESIGN §7.1 |

### M4.2 UI 组件与动效

| 编号 | 任务 | 优先级 | 状态 | 验收标准 | 关联规范 |
|---|---|---|---|---|---|
| M4.2.1 | 实现 `BtnPrimary`、`BtnSecondary`、`BtnGhost` 三种按钮 | P1 | 未开始 | 颜色、圆角、Hover/Press 效果符合 DESIGN §5.1–5.3 | DESIGN §5 |
...
| M4.3.2 | 存档异常处理 | P1 | 未开始 | 存档失败时显示错误弹窗，不崩溃 | DESIGN §7.7 |
