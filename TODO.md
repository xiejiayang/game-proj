...
| M4.1.4 | 提示弹窗 | P1 | 已完成 | 老河工分层提示（L1 最多两层） | PRD §4 |
| M4.1.5 | 暂停菜单 | P1 | 已完成 | 继续、重试、设置、返回标题 | PRD §4 |
| M4.1.6 | 结算/碑廊弹窗 | P0 | 已完成 | 两屏弹窗：屏1 结果反馈，屏2 碑廊「堵不如疏」图文 | PRD §4 |
| M4.1.7 | 设置页 | P1 | 已完成 | 从暂停菜单进入；音乐、音效、画质、语言 | PRD §4 |
...
| M4.2.2 | 实现 `Card`、`ToolbarItem`、`ResourceDot`、`Seal`、`DialogOverlay`、`HintPill` 组件 | P1 | 已完成 | 尺寸、颜色、动画符合 DESIGN §5 | DESIGN §5 |
...
| 编号 | 任务 | 优先级 | 状态 | 验收标准 | 关联规范 |
|---|---|---|---|---|---|
| M4.3.1 | 本地存档可读可写 | P0 | 已完成 | 保存/读取 PlayerProfile、关卡进度、设置；WebGL 使用 localStorage | PRD §3.2、ARCH §4.2 |
